...
void USART_Recieve(unsigned char* buffer, unsigned int size)
{
...
void move_servo(unsigned int ID, unsigned int Angle)
{
	...
	unsigned int header = 0xFF; //B�rja varje med 2st 0xFF
...
void load_servo(unsigned int ID, unsigned int Angle)
{
	...
	unsigned int P1 = 30; // Address att skriva till
	unsigned char P2 = (unsigned char)Angle; // f�rsta byten i m�lpositionen, minst signifikant
	unsigned char P3 = (unsigned char)(Angle>>8); // andra byten i m�lpositionen
	unsigned int Checksum = ~(ID + Length + Instruction + P1 + P2 + P3);
...
void set_speed(unsigned int ID, unsigned int speed)
{
	...
	unsigned char P2 = (unsigned char)speed; // f�rsta byten i m�lpositionen, minst signifikant
	unsigned char P3 = (unsigned char)(speed>>8); // andra byten i m�lpositionen
	unsigned int Checksum = ~(ID + Length + Instruction + P1 + P2 + P3);
